...
| Q2 | "Import all" — merge into existing lessons or replace the whole list? | **Decided: merge with fresh ids.** | `importLesson` already mints new ids, so merge is non-destructive *and* free — no id collisions, no "replace" path to guard. See [plans/001](../plans/done/001-data-durability.md) Task B. |
